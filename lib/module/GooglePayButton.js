...
import {
// NativeModules,
requireNativeComponent, TouchableOpacity, StyleSheet, Platform } from 'react-native';
import uuid from 'react-native-uuid';
import { isReadyToPay, init, loadPaymentData } from "./index.js";
import { EstonianDefaults } from "./types.js";
import { getMerchantInfo, openEPSession, processPayment } from "./EveryPayUtil/EveryPayRequests.js";
import { EveryPayGooglePayError } from "./everyPayError.js";
import { ERROR_CODES } from "./constants.js";

// @ts-ignore
import { jsx as _jsx } from "react/jsx-runtime";
const isFabricEnabled = global.nativeFabricUIManager != null;
const NativeGooglePayButton = isFabricEnabled ? require('./specs/GooglePayButtonNativeComponent').default : requireNativeComponent('EveryPayGooglePayButton');
const GooglePayButton = ({
  ...
  const [EPSessionInfo, setEPSessionInfo] = useState(null);
  const initGooglePay = async () => {
    ...
    try {
      await init(config.environment, config?.allowedCardNetworks || ['MASTERCARD', 'VISA'], config?.allowedCardAuthMethods || ['PAN_ONLY', 'CRYPTOGRAM_3DS']);
      const readyToPay = await isReadyToPay();
      const body = {
        api_username: config.apiUsername,
        account_name: 'EUR3D1'
      };
      const sessionInfo = await openEPSession(config.apiUrl, config.apiUsername, config.apiSecret, body);
      ...
      onPressCallback?.({
        state: 'failed',
        error: new EveryPayGooglePayError(ERROR_CODES.GOOGLE_PAY_INITIALIZATION_FAILED, error?.message)
      });
    }
  };
  useEffect(() => {
    ...
    return [{
      ...
      tokenizationSpecification: {
        ...
        parameters: {
          ...
          gatewayMerchantId: EPSessionInfo.googlepay_merchant_identifier.toLowerCase()
        }
      }
    }];
  };
  const onPress = async () => {
    ...
    try {
      ...
      merchantInfo = await getMerchantInfo(config.apiUrl, config.apiUsername, config.apiSecret, body);
    } catch (error) {
      console.error('Error making merchant info request', error);
      onPressCallback?.({
        state: 'failed',
        error: new EveryPayGooglePayError(ERROR_CODES.MERCHANT_INFO_REQUEST_ERROR, error?.message)
      });
    }
    if (!EPSessionInfo) {
      await initGooglePay();
    ...
    try {
      const googlePayRequest = {
        ...
        allowedPaymentMethods: [{
          ...
          tokenizationSpecification: {
            ...
            parameters: {
              gateway: EPSessionInfo.google_pay_gateway_id.toLowerCase(),
              gatewayMerchantId: EPSessionInfo.googlepay_gateway_merchant_id.toLowerCase()
            }
          }
        }],
        merchantInfo: {
      ...
      const token = JSON.parse(JSON.parse(paymentData).paymentMethodData.tokenizationData.token);
      ...
      const paymentProcessResponse = await processPayment(config.apiUrl, merchantInfo?.mobile_access_token || '', paymentProcessRequestBody);
      if (onPressCallback) {
        onPressCallback({
      ...
      if (e.code && e.message) {
        console.error('GooglePayButton error', `code: ${e.code}, message: ${e.message}`);
        onPressCallback?.({
          state: 'failed',
  ...
  return /*#__PURE__*/_jsx(TouchableOpacity, {
    ...
    children: /*#__PURE__*/_jsx(NativeGooglePayButton, {
